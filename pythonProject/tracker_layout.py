...
def create_tracker_tabs(path: str, maps_names:list):
    ...
    with open(path+"/layouts/tabs.json", 'w') as tabs:
        tabbed_maps_horizontal = {
            "type": "tabbed",
            "tabs": [

            ]
        }
        tabbed_maps_vertical = {
            "type": "tabbed",
            "tabs": [
...
def create_item_layout(path: str):
    ...
    with open(path + "/layouts/items.json", 'w') as item_layout:
        ...
        for i, item in enumerate(item_codes):
            # item_layout.write('\n[\n')
            if (i+1) % divider == 1:
                item_layout_json["shared_item_grid_horizontal"]["content"][0]["content"][0]["rows"].append([])
                item_layout_json["shared_item_grid_vertical"]["content"][0]["content"][0]["rows"].append([])
            item_layout_json["shared_item_grid_horizontal"]["content"][0]["content"][0]["rows"][i//divider].append(item)
            item_layout_json["shared_item_grid_vertical"]["content"][0]["content"][0]["rows"][i//divider].append(item)
